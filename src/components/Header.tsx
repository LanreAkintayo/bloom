...
export default function Header({
  ...
  // Close desktop dropdown when path changes (good UX)
  useEffect(() => {
      ...
      <div className="w-full px-4 md:px-0 flex justify-between items-center h-14 md:h-16">
        ...
        <nav className="hidden lg:flex items-center gap-6 text-sm text-white/70">
          {navItems
            ...
            .map((item) => {
              if (item.type === "link") {
                // Check if this simple link is active
                ...
                return (
                  <Link
                    ...
                    className={`hover:text-white transition-colors font-medium ${
                      isActive ? "text-emerald-400" : "" // Active style
                    }`}
                  >
                    {item.label}
                  </Link>
                ...
                return (
                  <div key={item.label} className="relative">
                    <button
                      ...
                      // Apply active style to parent dropdown
                      className={`flex items-center gap-1 hover:text-white cursor-pointer font-medium transition-colors ${
                        isDropdownActive ? "text-emerald-400" : ""
                      }`}
                    >
                      {item.label}
                      <svg
                        ...
                        {item.dropdownItems?.map((sub) => {
                          ...
                          return (
                            <Link
                              key={sub.label}
                              href={sub.href}
                              // Apply active style to sub-item
                              className={`block px-3 py-2 hover:bg-slate-800 text-sm transition-colors ${
                                isSubActive
                                  ? "text-emerald-400 font-medium" // Active sub-item style
                                  : ""
                              }`}
                  ...
                  return (
                    <Link
                      ...
                      className={`flex items-center gap-2 px-4 py-2 hover:bg-slate-800 rounded transition-colors ${
                        isActive ? "bg-slate-800 text-emerald-400" : "" // Active style for mobile
                      }`}
                    ...
                    <div key={item.label} className="border-t border-slate-800">
                      <button
                        ...
                        className={`w-full flex justify-between items-center px-4 py-2 hover:bg-slate-800 rounded transition-colors font-medium ${
                          isDropdownActive ? "bg-slate-800 text-white" : "" // Active parent style
                        }`}
                      >
                            ...
                            return (
                              <Link
                                ...
                                className={`block px-6 py-2 hover:bg-slate-800 rounded text-sm transition-colors ${
                                  isSubActive ? "text-emerald-400" : "" // Active sub-item
                                }`}
